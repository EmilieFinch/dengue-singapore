...
tscv_predictions_weekly <- function(data_input, # Data frame
                                    ...
                                    include_lag_cases, # Either: "lag-cases", "no-lag-cases"
                                    filename) { # Character string of output file name
  start_time <- Sys.time()
  
  # Set up formulae for fitting and prediction
  
  if(include_lag_cases == "yes"){
    sero_clim_form <- "+f(inla.group(time_since_switch, n = 18), model = 'rw2', scale.model = TRUE, hyper = precision_prior)+f(inla.group(max_t_scale_12_wk_avg_0),model = 'rw2', scale.model = TRUE, hyper = precision_prior)+f(inla.group(nino34_12_wk_avg_4, n = 12), model = 'rw2', scale.model = TRUE, hyper = precision_prior)+f(inla.group(days_no_rain_12_wk_total_0),model = 'rw2', scale.model = TRUE, hyper = precision_prior) + log(lag_cases)"
    sero_form <- "+f(inla.group(time_since_switch, n = 18), model = 'rw2', scale.model = TRUE, hyper = precision_prior) + log(lag_cases)"
    clim_form <- "+f(inla.group(max_t_scale_12_wk_avg_0),model = 'rw2', scale.model = TRUE, hyper = precision_prior)+f(inla.group(nino34_12_wk_avg_4, n = 12), model = 'rw2', scale.model = TRUE, hyper = precision_prior)+f(inla.group(days_no_rain_12_wk_total_0),model = 'rw2', scale.model = TRUE, hyper = precision_prior)+log(lag_cases)"
  } else if(include_lag_cases == "no"){
    ...
    print("lag_cases input not recognised.")
  }
  baseline <- ""
  baseline_year <- ""
  ...
  if (form_input == "all") {
    ...
    print("Inputted formula not recognised")
  }
  
  # Set variables to use for prediction
  if (horizon == 0) {
    predict_vars <- c("time_since_switch", "max_t_scale_12_wk_avg_0", "nino34_12_wk_avg_4", "days_no_rain_12_wk_total_0", "lag_cases")
  } else if (horizon == 2) {
    predict_vars <- c("time_since_switch_2", "max_t_scale_10_wk_avg_2", "nino34_12_wk_avg_4", "days_no_rain_10_wk_total_2", "lag_cases_2")
  } else if (horizon == 4) {
    predict_vars <- c("time_since_switch_4", "max_t_scale_8_wk_avg_4", "nino34_12_wk_avg_4", "days_no_rain_8_wk_total_4", "lag_cases_4")
  } else if (horizon == 6) {
    predict_vars <- c("time_since_switch_6", "max_t_scale_6_wk_avg_6", "nino34_10_wk_avg_6", "days_no_rain_6_wk_total_6", "lag_cases_6")
  } else if (horizon == 8) {
    predict_vars <- c("time_since_switch_8", "max_t_scale_4_wk_avg_8", "nino34_8_wk_avg_8", "days_no_rain_4_wk_total_8", "lag_cases_8")
  }
  
  cat(c("Using these variables for prediction: ", predict_vars), ". Include lagged cases as predictors:", include_lag_cases, "\n")
    ...
    for (n in 471:1200) { # Should be 471:1200 (using Jan 2000 to Jan 2009 as training data)
      ...
      df_pred <- data_input |>
        ...
        mutate(
          ...
          lag_cases = case_when(
            date_index > n - horizon & date_index != n ~ NA_integer_,
            TRUE ~ get(predict_vars[5]) # for lagged cases train with appropriate lag for forecast horizon
          )
        )
      ...
      if (horizon != 0) {
        df_pred <- df_pred |>
          mutate(
            ...
            lag_cases = case_when(date_index == n ~ get(predict_vars[5]), TRUE ~ lag_cases)
  ...
  saveRDS(temp_out, here("output", paste0(Sys.Date()), paste0(filename, "_horizon-", horizon, ".rds")))
  file.remove(here("output", paste0(Sys.Date()), paste0(filename, "_lag-", horizon, "_temp.rds")))
}

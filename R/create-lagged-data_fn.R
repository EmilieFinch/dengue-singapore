...
lag_data <- function(data) {
  ...
  df_model <- data |>
    ...
    mutate(year_index = year - 1999, date_num = round(as.numeric(date)), max_t_scale = max_t - mean(max_t), mean_t_scale = mean_t - mean(mean_t), min_t_scale = min_t - mean(min_t),
           ab_hum_scale = ab_hum - mean(ab_hum), prec_scale = prec - mean(prec, na.rm = TRUE), lag_cases = lag(cases)) # Note need to round as otherwise can't match numeric date columns
  ...
  df_model <- df_model |> 
    bind_cols(setNames(shift(df_model$cases, seq(2,8, by = 2)), c(paste0("lag_cases_", seq(2,8, by = 2))))) |> 
